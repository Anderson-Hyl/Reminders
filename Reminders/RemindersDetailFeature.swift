...
class RemindersDetailModel {
	...
	func toogleShowCompletedButtonTapped() async {
...
struct RemindersDetailView: View {
	...
	var body: some View {
		...
		.navigationTitle(Text(model.detailType.navitationTitle))
		.listStyle(.plain)
		.toolbar {
			...
			ToolbarItem(placement: .primaryAction) {
				Menu {
					Group {
						...
						Button {
							Task {
								await model.toogleShowCompletedButtonTapped()
							}
						} label: {
							Label {
								Text(
